...
-- Causa raiz real del "guardado colgado": STALE_SNAPSHOT usaba errcode
-- '40001' (serialization_failure), la clase estandar de Postgres para
-- conflictos transitorios de transacciones concurrentes. La capa de
-- pooling/API de Supabase trata ese codigo como "reintentable" y lo
-- reintenta automaticamente en segundo plano. Como nuestro conflicto de
-- revision NO es transitorio (la revision esperada simplemente esta
-- desactualizada), cada reintento fallaba exactamente igual, y el pooler
-- seguia reintentando durante mas de dos minutos antes de rendirse con
-- "upstream request timeout". Confirmado de forma reproducible: con la
-- revision correcta el guardado responde en ~1s; con una revision
-- incorrecta (que dispara este codigo) la llamada se queda colgada.
--
-- Se cambia a un codigo fuera de la clase 40 (transaction_rollback) para
-- que no se interprete como un conflicto reintentable. El codigo del lado
-- del cliente (save-admin-snapshot.js) ya reconoce este error tanto por
-- codigo como por el mensaje 'STALE_SNAPSHOT', asi que el cambio es seguro.

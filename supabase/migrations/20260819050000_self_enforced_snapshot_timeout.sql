...
-- Defensa adicional: los timeouts a nivel de rol/base de datos solo aplican a
-- conexiones NUEVAS. PostgREST mantiene un pool de conexiones de larga
-- duracion (no reconecta en cada request), asi que una conexion abierta
-- antes de "alter database ... set idle_in_transaction_session_timeout"
-- sigue sin limite hasta que esa conexion se cierre y se reabra. Eso explica
-- por que seguian apareciendo sesiones "idle in transaction" zombis incluso
-- despues de aplicar el limite a nivel de rol y de base de datos.
--
-- Aqui forzamos el limite DENTRO de la propia transaccion de
-- save_admin_snapshot, con set_config(..., true) (alcance local a la
-- transaccion). Esto no depende de que la conexion haya sido abierta antes o
-- despues de ningun ALTER: se aplica siempre, en cada llamada, sin importar
-- la antiguedad de la conexion del pool.
